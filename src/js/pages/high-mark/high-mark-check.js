...
$(function () {
    ...
    function init() {
        ...
        initTable();
        // 初始化表格搜索
        $.initSearchTable(_TABLE_OBJ, _HOST.add_rort + _HOST.student.high_mark.list_check);
        // 添加模块
        initAddModule();
    ...
    function initTable() {
        _TABLE_OBJ = $('#table').dataTable({
            ajax: {
                url: _HOST.add_rort + _HOST.student.high_mark.list_check,
                type: 'POST',
                dataSrc: (res) => {
                    return res.Data;
                }
            ...
            columns: [{
                    ...
                    render: (data, type, row, meta) => {
                        var result = `<button type="button" class="btn btn-default size-S btn-desc" data-modify='${JSON.stringify(row)}' title="详情">详情</button>`;
