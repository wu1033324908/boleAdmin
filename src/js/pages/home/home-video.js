...
$(function () {
    ...
    function initAddModule() {
        ...
        $('.tool-container').on('click', '.btn-add', () => {
            ...
            // 视频上传
            $('#add-video-uploader').on('uploaded', function (upload, file) {
                let response = JSON.parse(file.result.response);
                if (response.Result) {
                    fun_data['videoId'] = response.Id;
                    console.log(fun_data)
                }
            });
        ...
        function formSubmit(fun_data) {
            ...
            layui.form.on('submit(add)', function (data) {
                ...
                if (!fun_data.imageId) {
                    ...
                    sendData.videoid = fun_data.videoId;
                    ...
                    $.ajaxRequestTemplate({
                        url: _HOST.add_rort + _HOST.home.video.add,
                        data: sendData
                    })
                }
    ...
    function initModifyModule() {
        ...
        $('.dataTables_wrapper').on('click', 'tbody .btn-modify', (e) => {
            ...
            // 视频上传成功保存id
            $('#modify-video-uploader').on('uploaded', function (upload, file) {
                let response = JSON.parse(file.result.response);
                if (response.Result) {
                    fun_data['videoId'] = response.Id;
                }
        ...
        function formSubmit(fun_data) {
            ...
            layui.form.on('submit(modify)', function (data) {
                if (!fun_data.imageId) {
                    ...
                    $.ajaxRequestTemplate({
                        url: _HOST.add_rort + _HOST.home.video.modify,
                        data: sendData
                    })
                }
        ...
        function initForm(fun_data) {
            ...
            fun_data.videoId = fun_data.modify.VideoId;
