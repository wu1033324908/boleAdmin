...
$(function () {
    ...
    function init() {
        ...
        // 初始化表格搜索
        $.initSearchTable(_TABLE, _HOST.add_rort + _HOST.qa.list);
        // 初始化添加模块
        initAddModule();
        // 初始化修改模块
        initModifyModule();
    ...
    function initTable() {
        _TABLE = $('#table').dataTable({
            ...
            ajax: {
                url: _HOST.add_rort + _HOST.qa.list,
                type: 'POST',
                dataSrc: (res) => {
                    return res.Data;
            ...
            columns: [{
                    ...
                    render: (data, type, row, meta) => {
                        if(data){
                            return `
                                <button type="button" class="btn btn-default size-S btn-unhot" data-modify='${JSON.stringify(row)}' title="取消设为热点">取消设为热点</button>
                            ...
                            return `
                                <button type="button" class="btn btn-default size-S btn-hot" data-modify='${JSON.stringify(row)}' title="设为热点">设为热点</button>
                                <button type="button" class="btn btn-default size-S btn-modify" data-modify='${JSON.stringify(row)}' title="修改"><i class="fa fa-edit"></i></button>
                                <button type="button" class="btn btn-default size-S btn-delete" data-modify='${JSON.stringify(row)}' title="删除"><i class="fa fa-trash-o"></i></button>
                            `;
                        }
    ...
    function initAddModule() {
        ...
        function formSubmit(fun_data) {
            ...
            layui.form.on('submit(add)', function (data) {
                ...
                $.ajaxRequestTemplate({
                    url: _HOST.add_rort + _HOST.qa.add,
                    data: sendData
                })
    ...
    function initModifyModule() {
        ...
        function formSubmit(fun_data) {
            ...
            layui.form.on('submit(modify)', function (data) {
                ...
                $.ajaxRequestTemplate({
                    url: _HOST.add_rort + _HOST.qa.mod,
                    data: sendData
                })
                return false;
            });
        ...
        function initForm(fun_data) {
            ...
            // 课程名称
            form.find('input[name=question]').val(fun_data.modify.Question);
            // 排序值
            form.find('input[name=answer]').val(fun_data.modify.Answer);
        }
    }
    ...
    function initDeleteModule() {
        $('.dataTables_wrapper').on('click', 'tbody .btn-delete', (e) => {
            ...
            layui.layer.open({
                ...
                yes: (index, layero) => {
                    $.ajaxRequestTemplate({
                        url: _HOST.add_rort + _HOST.qa.del,
                        data: {
                            Id: Id
                        },
                        successMsg: '删除成功'
    ...
    function ishot() {
        $('.dataTables_wrapper').on('click', 'tbody .btn-hot', (e) => {
            let $this = $(e.currentTarget);
            console.log($this.data('modify'))
            let Id = $this.data('modify').Id;
            $.ajax({
                type: "post",
                url: _HOST.add_rort + _HOST.hot.set,
                data: {
                    isHost:true,
                    Id:Id
                },
        ...
        $('.dataTables_wrapper').on('click', 'tbody .btn-unhot', (e) => {
            let $this = $(e.currentTarget);
            console.log($this.data('modify'))
            let Id = $this.data('modify').Id;
            $.ajax({
