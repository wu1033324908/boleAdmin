...
$(function () {
    ...
    function init() {
        ...
        initTable();
        // 初始化表格搜索
        $.initSearchTable(_TABLE, _HOST.add_rort + _HOST.student.study_record_all);
        // 初始化修改模块
        initModifyModule();
    ...
    function initTable() {
        _TABLE = $('#table').dataTable({
            ajax: {
                url: _HOST.add_rort + _HOST.student.study_record_all,
                type: 'POST',
                dataSrc: (res) => {
                    return res.Data;
                }
    ...
    function initModifyModule() {
        ...
        function initForm(fun_data) {
            ...
            // 分数
            form.find('.score').text(fun_data.modify.Grade || '');
            // 手机号
            ...
            // 贴图
            form.find('.jigsaw-image').attr('src', fun_data.modify.PuzzlThumbnailUrl[0]);
            form.find('.jigsaw-image').attr('data-src', fun_data.modify.PuzzlUrl[0]);
