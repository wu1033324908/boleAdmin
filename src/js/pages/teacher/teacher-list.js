...
$(function () {
    ...
    function init() {
        ...
        initTable();
        // 初始化表格搜索
        $.initSearchTable(_TABLE, _HOST.add_rort + _HOST.teacher.list_all);
        // 初始化添加模块
        initAddModule();
        // 初始化修改模块
        initModifyModule();
    ...
    function initTable() {
        _TABLE = $('#table').dataTable({
            order: [[ 2, 'asc' ]],
            ajax: {
                url: _HOST.add_rort + _HOST.teacher.list_all,
                type: 'POST',
                dataSrc: (res) => {
                    return res.Data;
                }
    ...
    function initModifyModule() {
        ...
        function initForm(fun_data) {
            ...
            // 排序值
            form.find('input[name=sortcode]').val(fun_data.modify.SortCode);
        }
    }
